...
mkCalendar f files =
  Calendar
    ...
    . f
    . concatMap (uncurry walkEntries)
    <$> traverse parse files
  where
    parse file = (toName file,) <$> OrgParser.parseOrgDocIO OrgParser.defaultOrgOptions file
    toName file = T.pack $ file ^. basename
...
walkEntries :: Text -> Org.OrgDocument -> [Entry]
walkEntries source = Org.query go -- TODO: does this go deep?
  where
