...
processUpcoming cal showRecent n = do
  now <- C.now
  let result = fmap (C.fromEntry now) . C.entriesAfter n now showRecent $ cal
...
runNotifications config tcal = do
  when config.notification.enable do
    ...
    forever do
      ...
      let newEntries = C.entriesAfter numThreads now False cal
  ...
  where
    ...
    toMicroseconds now tsd =
      case Time.nominalDiffTimeToSeconds $ Time.diffLocalTime tsd now of
        Time.MkFixed i ->
          let result = i `div` 1_000_000
           in max 1_000_000 (result - 15 * 60_000_000)
...
runDaemon tcal = Network.withSocketsDo do
  ...
  where
    ...
    sendEntry conn entry = do
      let result = T.decodeUtf8 . BS.toStrict . Aeson.encode $ entry
      TF.useAsPtr (result <> "\n") $ \ptr len ->
    ...
    broadcastTimer broadcast tskip = do
      ...
      forever do
        ...
        let result = safeLast $ C.entriesAfter (skip + 1) now False cal
...
runExport Config.EctConfig {..} =
  let shouldExportCalendars = (T.unpack . Config.path <$> filter Config.shouldExport calendars)
   in Exporter.exportFiles shouldExportCalendars (T.unpack $ Config.outputPath export)
